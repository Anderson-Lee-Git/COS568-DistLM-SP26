...
def train(args, train_dataset, model, tokenizer):
    ...
    for _ in train_iterator:
        ...
        sched = schedule(skip_first=1, wait=0, warmup=1, active=3, repeat=1)
        with profile(
            activities=[ProfilerActivity.CPU],
            schedule=sched,
            on_trace_ready=lambda p: p.export_chrome_trace(
                f"{args.output_dir}/trace_{args.local_rank}.json"
            ),
        ) as prof:
            for step, batch in enumerate(epoch_iterator):
                ...
                def sync_gradients(model, args):
                    # worker 0 collect gradients using torch.distributed.gather
                    for param in model.parameters():
                        ...
                        if args.local_rank == 0:
                            gathered_grad = [
                                torch.zeros_like(local_grad) for _ in range(args.world_size)
                            ]
                        else:
                            gathered_grad = None
                        torch.distributed.gather(local_grad, gathered_grad, dst=0)
                        # average gradients
                        if args.local_rank == 0:
                            averaged_grad = torch.stack(gathered_grad).mean(dim=0)
                            scattered_grad = [
                                averaged_grad.clone() for _ in range(args.world_size)
                            ]
                        else:
                            scattered_grad = None
                        torch.distributed.scatter(param.grad, scattered_grad, src=0)
                ...
                if (step + 1) % args.gradient_accumulation_steps == 0:
                    ...
                    global_step += 1
                prof.step()
                training_time.append(timer.stop())
    ...
    return global_step, tr_loss / global_step, training_time, loss_curve
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=12345,
        help="Port number for communication between nodes",
    )
    ...
    if args.do_train:
        ...
        global_step, tr_loss, training_time, loss_curve = train(
            args, train_dataset, model, tokenizer
        )
        logger.info(" global_step = %s, average loss = %s", global_step, tr_loss)
        with open(
            os.path.join(args.output_dir, f"training_time_{args.local_rank}.json"), "w"
        ) as f:
            json.dump(training_time, f)
        with open(
            os.path.join(args.output_dir, f"loss_curve_{args.local_rank}.json"), "w"
        ) as f:
